...
public class ContactsProvider {
  ...
  public WritableMap getContactAfterPick(Uri uri) {
    ...
    try {
      if (cursor.moveToFirst()) {
        ...
        String phone_number = cursor.getString(phoneIndex);
        String full_name = cursor.getString(nameIndex);
        map.putString("full_name", full_name);
        map.putString("phone_number", phone_number);
        return map;
      }
      return null;
    } finally {
  ...
  public WritableArray getContacts(ReadableArray scope) {
    WritableArray contacts;
    ArrayList args = scope.toArrayList();
    Boolean hasEmail = args.contains("email");
    Boolean hasPhone = args.contains("phone");

    if (hasEmail && hasPhone) {
      contacts = getContactsByAll();
    } else if (hasEmail && !hasPhone) {
      contacts = getContactsByEmail();
    } else {
      contacts = getContactsByPhone();
    }
    return contacts;
  }

  public WritableArray getContactsByAll() {
    WritableArray contacts = Arguments.createArray();
    {
      Cursor cursor = contentResolver.query(
        Phone.CONTENT_URI,
      ...
      try {
        while (cursor != null && cursor.moveToNext()) {
          ...
          String full_name = cursor.getString(cursor.getColumnIndex(ContactsContract.Data.DISPLAY_NAME));
          String email = getEmail(contactId);
          map.putString("id", String.valueOf(contactId));
          map.putString("full_name", full_name);
          map.putString("email", String.valueOf(email));
          map.putString("phone_number", String.valueOf(phone_number));
          contacts.pushMap(map);
        }
      } finally {
        if (cursor != null) {
          cursor.close();
  ...
  private String getEmail(int contactId) {
    ...
    final Cursor email = contentResolver.query(Email.CONTENT_URI, projection, Email.CONTACT_ID + "=" + contactId, null, null);
  ...
  public WritableArray getContactsByEmail() {
    WritableArray contacts = Arguments.createArray();
    {
      Cursor cursor = contentResolver.query(
        Email.CONTENT_URI,
  ...
  public WritableArray getContactsByPhone() {
    WritableArray contacts = Arguments.createArray();
    {
      Cursor cursor = contentResolver.query(
        Phone.CONTENT_URI,
